...
- Enabled Vercel Web Analytics and Speed Insights — free Hobby-tier traffic and Core Web Vitals tracking across all sources, not just Google Search.
...
- Removed `cleanUrls: true` from vercel.json — it was intercepting requests for non-prerendered paths and returning 404 before the SPA rewrite could match. All prerendered pages live at `dir/index.html` so Vercel's automatic directory-index handling keeps them working without cleanUrls.

## [2026-05-06]

### Changed

- Reduced daily AI query limit from 7 to 5. VIP bypass unchanged.

## [2026-05-05]

### Changed

- **Browse mode: documents are now grouped by tradition (Ecumenical, Reformed, Lutheran, Baptist, Anglican, Catholic, Orthodox) instead of a flat list. Header strip simplified to tradition names. Improves mobile usability and scales as more documents are added.** The Browse sidebar now renders each tradition as a collapsible section heading (alphabetical, matching the SHOW: filter chips) with its documents listed underneath (alphabetical by displayed name). The section containing the active document is force-expanded. The compact identity strip under the app title was reduced from a full doc list to tradition names only — clicking a tradition there opens Browse on the first doc of that tradition. The `/all-documents` index page mirrors the same hierarchy: each tradition gets a colored heading with its documents grouped beneath. Grouping is data-driven from each document's `tradition` tag, so adding a new doc (e.g., Schleitheim → Anabaptist) slots into the right group automatically. EN/DE locales preserved; deep-links unchanged.

### Added

- **Joint Declaration on the Doctrine of Justification (JDDJ, 1999) added under the Ecumenical tradition.** Links out to the official Vatican-hosted text in EN and DE; not embedded due to copyright. Particularly relevant for justification queries — fills a gap in the Ecumenical column where the creeds don't address justification. Implementation mirrors the modern CCC pattern: new canonical doc_id `jddj` registered in `src/utils/anchors.js` (DOC_IDS, EXTERNAL_DOC_IDS, EN_NAME_TO_DOC_ID, DE_NAME_TO_DOC_ID, DOC_ID_ALIASES — including "JDDJ", "Joint Declaration", "Gemeinsame Erklärung zur Rechtfertigungslehre", "GER"), with a new `external` schema entry keyed by `paragraph` 1–44. New `src/utils/jddj.js` resolver returns the language-appropriate Vatican URL (christianunity.va, the Pontifical Council for Promoting Christian Unity's site — the older `vatican.va/roman_curia/pontifical_councils/chrstuni/...` URLs from the original task brief 404; christianunity.va is the current canonical home). The Vatican page does NOT include per-paragraph anchors (only footnote anchors), so the resolver returns the document URL and the reader scrolls to the cited numbered paragraph; the paragraph argument is still validated for range. Both `RESEARCH_JSON_PROMPT` and `COMPARISON_PROMPT` updated: `jddj` added to canonical doc_id whitelist and per-doc range guide; new section "4b" tells the model JDDJ is a 1999 ecumenical agreement (Lutheran, Catholic, Methodist, Reformed, Anglican) on justification, central paragraphs ¶14–39, tag `tradition: "Ecumenical"` and `doc_id: "jddj"`, omit verbatim quotes if not confident; new section "9" in Compare prompt directs the model to prefer JDDJ over a creed for the Ecumenical cell on aspects the creeds don't address (justification, faith and works, salvation by grace). Standard-locus guide updated: Justification → JDDJ ¶14–39 added alongside Westminster Ch. 11, Augsburg Art. IV, 1689 Ch. 11, Heidelberg Q60, Belgic Art. 22–23, Helvetic2 Ch. 15, 39 Articles Art. XI, CCC 1987–2029 (modern), Roman Catechism Ch. 19. Render side (src/App.jsx): citation cards in Research and citation cells in Compare detect `isJDDJDocId(docId)` alongside `isCCCModernDocId(docId)` under `EXTERNAL_DOC_IDS.has(docId)` and route through `getJDDJUrl(paragraph, lang)` for the outbound link. Regex fallback now extracts paragraph numbers from "JDDJ ¶15" / "JDDJ 14–18" style citation strings, with the right max bound per doc_id (44 for JDDJ, 2865 for CCC). Browse integration: new `EXTERNAL_BROWSE_DOCS` registry in App.jsx (with EN/DE display names and descriptions) drives a Browse-sidebar entry under Ecumenical with an outbound marker (↗); clicking it sets `selectedConfession` to the sentinel "external:jddj" and the main pane renders a stub page (title, year, tradition, description, prominent EN and DE outbound links to vatican.va). `openBrowseAt` short-circuits external doc_ids to the stub. Path-based deep links (`/browse/jddj`, `/de/browse/jddj`) resolve through the existing route handler. Prerender (`scripts/prerender.mjs`): new EXTERNAL_DOCS array drives stub pages for `/browse/jddj` and `/de/browse/jddj` with title, description, hreflang alternates, and Schema.org Book JSON-LD pointing `sameAs` at the official Vatican URL; both URLs added to sitemap.xml. New i18n strings (`jddjExternalTooltip`, `externalDocBanner`, `readEnglish`, `readGerman`, `linkedNotEmbedded`, `externalDocTooltip`) in EN and DE; About modal `aboutCCCNote` extended to mention JDDJ. README "Source Texts Included" gains an "Ecumenical Reference Documents (linked, not embedded)" subsection listing JDDJ. Sample resolved URLs (200 OK): EN → `https://www.christianunity.va/.../1999-dichiarazione-congiunta-sulla-dottrina-della-giustificazion/en.html`; DE → `https://www.christianunity.va/.../en/de.html`.

- **Outbound deep-linking to the modern Catechism of the Catholic Church (1992) on vatican.va.** The CCC text is not embedded (Libreria Editrice Vaticana / USCCB / Deutsche Bischofskonferenz copyright; >5000-word use requires written permission), but Compare and Research can now cite specific CCC paragraphs and link directly to the corresponding passage on the Vatican website in EN or DE. Roman Catechism (1566) embedded coverage continues unchanged. Implementation: new `src/utils/ccc.js` resolver maps a CCC paragraph (1–2865) to the closest IntraText archive page (`https://www.vatican.va/archive/ENG0015/__P{XX}.HTM` for English, `/archive/DEU0035/` for German) using a sparse, language-specific lookup table — vatican.va groups consecutive paragraphs onto sequential pages with no per-paragraph anchors, so the resolver lands the user on the page containing or starting just before the cited paragraph. New canonical doc_id `ccc-modern` added to `DOC_IDS` in `src/utils/anchors.js`, marked as `external: true` via the new `EXTERNAL_DOC_IDS` set so the link builder routes it through `getCCCUrl()` instead of `buildBrowsePath()`. Aliases added: "CCC", "Catechism of the Catholic Church", "Katechismus der Katholischen Kirche", "KKK", "1992 Catechism", "modern catechism". New `external` schema in `DOC_SCHEMAS` keyed by `paragraph` with range 1–2865; `normalizeLocation` short-circuits external docs to a `{paragraph}` shape and `collectLocationKeys` now treats `paragraph` as a distinct axis (no longer folded into `section`). Both `RESEARCH_JSON_PROMPT` and `COMPARISON_PROMPT` updated to add `ccc-modern` to the canonical doc_id whitelist and the per-doc schema list, with a new "two valid Catholic sources" section telling the model to pick whichever is most directly on-topic for the user's question — common loci documented (Trinity → CCC 232–267; Christology → CCC 422–682; Eucharist → CCC 1322–1419; Baptism → CCC 1213–1284; Justification → CCC 1987–2029; Church → CCC 748–810; Sacraments → CCC 1113–1134; Ten Commandments → CCC 2052–2557; Prayer → CCC 2558–2865). The two doc_ids cannot be confused: ccc-modern uses `{paragraph: N}`, roman-catechism uses `{chapter, section}` — different schemas, different documents. Render side: in `src/App.jsx`, both the Research citation card and the Compare cell citation now detect `EXTERNAL_DOC_IDS.has(docId)` and render an `<a href={getCCCUrl(paragraph, lang)} target="_blank" rel="noopener noreferrer">` instead of the in-app `<button>` — outbound links are visually distinct (small "vatican.va ↗" affordance) and tooltip-tagged "Modern Catechism of the Catholic Church (1992) — opens at vatican.va". For Research, a regex fallback (`CCC\s*(\d+)`) extracts a paragraph number from the human-readable `reference`/`quote` when the model emits a citation string but forgets the structured `paragraph`. New i18n strings `cccExternal` and `cccExternalTooltip` in `src/i18n/en.js` and `src/i18n/de.js`, plus `aboutCCCNote` describing the policy. README and About modal updated.

## [2026-05-02]

### Added

- **Belgic Confession (1561), Canons of Dort (1619), and Second Helvetic Confession (1566) added under the Reformed tradition.** Completes the Three Forms of Unity (Heidelberg Catechism, Belgic Confession, Canons of Dort) plus the Helvetic tradition for Continental Reformed users. Each document is fully integrated: English text in `src/data/confessions.js`, German text in `src/data/confessions_de_extra.js`, doc_ids (`belgic`, `dort`, `helvetic2`) registered in `src/utils/anchors.js` (DOC_IDS, EN_NAME_TO_DOC_ID, DE_NAME_TO_DOC_ID, DOC_ID_ALIASES, DOC_SCHEMAS), added to the canonical doc_id whitelists in `RESEARCH_JSON_PROMPT` and `COMPARISON_PROMPT`, standard-locus guidance added (Election→Dort I; Atonement→Dort II; Total Depravity→Dort III/IV; Perseverance→Dort V; Scripture→Belgic Art. 2–7, Helvetic2 Ch. 1–2; Church→Belgic Art. 27–32, Helvetic2 Ch. 17; Sacraments→Belgic Art. 33–35, Helvetic2 Ch. 19–21; Trinity→Belgic Art. 8–9, Helvetic2 Ch. 3), i18n strings updated in EN and DE, and EN→DE key mapping added to `scripts/prerender.mjs` so German pages are prerendered. Belgic schema: `chapter-as-N`, 37 articles. Dort schema: `chapter+section`, chapters 1–5 (head 3/4 combined as chapter 3). Helvetic2 schema: `chapter+section`, 30 chapters.
...
- **Traditions/denominations are now ordered alphabetically (per locale) in all UI lists.** Reordered `ALL_TRADITIONS` in `src/data/confessions.js` from the previous historical/grouping order (Reformed, Lutheran, Catholic, Baptist, Ecumenical, Orthodox, Anglican) to alphabetical (Anglican, Baptist, Catholic, Ecumenical, Lutheran, Orthodox, Reformed). Because the Research filter chips, Compare column headers, and citation tradition labels all derive from this single array, the change propagates everywhere the user sees a tradition list — Research "SHOW:" filter bar, Compare table column order, Sources sidebar grouping. The "All" button in the Research filter remains at the end as a separator, unchanged in position. The prose tradition lists in About and the welcome intro (`welcomeIntro`, `aboutDescription`) are also alphabetized — by English label in `en.js` (Anglican, Baptist, Catholic, Lutheran, Orthodox, Reformed) and by German label in `de.js` (anglikanisch, baptistisch, katholisch, lutherisch, orthodox, reformiert), with the Ecumenical Creeds kept as the trailing special case in both locales since they are creeds shared across all traditions rather than a denominational tradition. No data files, prompt logic, or filter-matching code was touched — only rendering order. Colors/styling of chips and column headers are unchanged.
